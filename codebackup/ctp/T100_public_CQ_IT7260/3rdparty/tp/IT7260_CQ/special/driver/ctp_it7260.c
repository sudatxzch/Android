/*zhangzetao 2012-5-17 modify to solve TYLL-NO,the disable to wake up on chuangqi*/
/* 
...
#define PIXCIR_DEBUG
#ifdef PIXCIR_DEBUG
//#define IT7260_DBG(format, ...)	\
//		printk(KERN_INFO "PIXCIR_TS " format "\n", ## __VA_ARGS__)
#define IT7260_DBG(format, ...)	 printk(format)
//		printk(KERN_INFO "PIXCIR_TS " format "\n", ## __VA_ARGS__)
#else
#define IT7260_DBG(format, ...)
#endif
...
static int ctp_it7260_irq;
static struct early_suspend	pixcir_early_suspend;

static void ctp_it7260_ts_suspend(struct early_suspend *handler);
static void ctp_it7260_ts_resume(struct early_suspend *handler);
static void ctp_it7260_ts_pwron(void);
static void ctp_it7260_ts_pwroff(void);


static int ctp_it7260_i2c_rxdata(char *rxdata, int length)
{
        int ret;
        int i;
        struct i2c_msg msgs[] = {
...
static int ctp_it7260_i2c_txdata(char *txdata, int length)
{
		int ret,i;
		struct i2c_msg msg[] = {
			{
...
int i2cWriteToIt7260( unsigned char bufferIndex, unsigned char const dataBuffer[], unsigned char dataLength)
{
 	...
 	return ctp_it7260_i2c_txdata( buf, dataLength+1 );	
}
...
int i2cReadFromIt7260( unsigned char bufferIndex, unsigned char dataBuffer[], unsigned char dataLength)
{
	...
	return ctp_it7260_i2c_rxdata( dataBuffer, dataLength);
}

static void ctp_it7260_ts_suspend(struct early_suspend *handler)
{
        unsigned char buffer[]={0x04,0x00,0x02};
	IT7260_DBG("==%s==\n", __func__);
	disable_irq_nosync(this_client->irq);

	i2cWriteToIt7260( 0x20, buffer, 3);

   	ctp_it7260_ts_pwroff();
}
...
{	
    ...
    gpio_direction_output(sprd_3rdparty_gpio_tp_irq,0);	
    ...
    {
        int ret;
	    ret = i2cReadFromIt7260( 0x80, &ucQuery, 1);
        if(ret)
	    break;
    }
...
static void ctp_it7260_ts_pwron(void)
{
	IT7260_DBG("=======================================ctp_it7260_ts_pwron++");
...
static void ctp_it7260_ts_pwroff(void)
{
	IT7260_DBG("=======================================ctp_it7260_ts_pwroff++");
...
static int  pixcir_ts_config_pins(void)
{
    ...
    gpio_direction_input(sprd_3rdparty_gpio_tp_irq);	
    gpio_direction_output(sprd_3rdparty_gpio_tp_rst,1);	
    ctp_it7260_irq=sprd_alloc_gpio_irq(sprd_3rdparty_gpio_tp_irq);
    return ctp_it7260_irq;
}
...
/*TYLL-NO. the disable to wake up on anda Modify By Zhu Caihua 2012-4-17 Start*/
static void pixcir_ts_poscheck(struct pixcir_i2c_ts_data *data)
{
	...
	rdbuf[0]=0x80;
	ctp_it7260_i2c_rxdata(rdbuf, 1);

	if (rdbuf[0]&0x01) // IC busy or not
	{
        enable_irq(this_client->irq);
        IT7260_DBG("ctp IT7260 IC busy");
        return IRQ_HANDLED;
	}
	else
	{
		...
		{
			rdbuf[0]=0xE0;
		      ret = ctp_it7260_i2c_rxdata(rdbuf, 14);
			//for (i=0;i<14;i++)
			//{
		 	...
		 	//if (ret)
			{
					...
					if((rdbuf[0]&0x40) && (rdbuf[0]&0x01))
					{
						if(rdbuf[2])//button down
						{
							...
							case 4:
					           ...
					           IT7260_DBG("virtual key down x=%d,y=%d \n",pos[0],pos[1]);
						...
						{
#if 1
					           IT7260_DBG("virtual key up x=%d,y=%d \n",pos[0],pos[1]);
						     switch(rdbuf[1])
							{
							case 1:
								pos[0]=40;
				...
				{
					IT7260_DBG("no more data push pen up\n");
				...
				{
                        IT7260_DBG("3point detect,enable irq and return");
                        enable_irq(this_client->irq);
                        return IRQ_HANDLED;
				}
				...
				if(rdbuf[0]&0x01)//4// Finger 1
				{
					...
					IT7260_DBG("push pen 1 down x=%d,y=%d\n",xraw,yraw);
				...
				if(rdbuf[0]&0x02)//4// Finger 2
				{
					...
					IT7260_DBG("push pen 2 down x=%d,y=%d\n",xraw,yraw);
					input_report_abs(tsdata->input, ABS_MT_POSITION_X,  xraw);
					input_report_abs(tsdata->input, ABS_MT_POSITION_Y,  yraw);
					input_report_abs(tsdata->input, ABS_MT_TOUCH_MAJOR, 200);
					input_report_abs(tsdata->input, ABS_MT_WIDTH_MAJOR, 1);
...
/*TYLL-NO. the disable to wake up on anda Modify By Zhu Caihua 2012-4-17 end*/

static irqreturn_t ctp_it7260_ts_isr(int irq, void *dev_id)
{
    struct pixcir_i2c_ts_data *tsdata = dev_id;
    ktime_t expires = ktime_set(0, 10000000UL);
    int ret = -1;

    IT7260_DBG("ctp_it7260_ts_isr ++");
...
static int pixcir_i2c_ts_suspend(struct device *dev)
{
    ...
    IT7260_DBG("pixcir_i2c_ts_suspend ++");
...
static int pixcir_i2c_ts_resume(struct device *dev)
{
    ...
    IT7260_DBG("pixcir_i2c_ts_resume ++");
...
//	struct i2c_client *client = to_i2c_client(dev);
	...
	// Identify Cap Sensor
	do {
		i2cReadFromIt7260(0x80, &ucQuery, 1);
	} while (ucQuery & 0x01);
	buffer[0] = 0x00;
	i2cWriteToIt7260( 0x20, buffer, 1);
	do {
		i2cReadFromIt7260( 0x80, &ucQuery, 1);
	} while (ucQuery & 0x01);

	memset(&buffer, 0, sizeof(buffer));
	i2cReadFromIt7260( 0xA0, buffer, 8);
	IT7260_DBG("=IT7260_Init1 --[%x][%x][%x][%x][%x][%x]=\n", buffer[0], buffer[1],
	...
	buffer[0] = 0x01;
	buffer[1] = 0x00;
	i2cWriteToIt7260( 0x20, buffer, 2);
	do {
		i2cReadFromIt7260( 0x80, &ucQuery, 1);
	} while (ucQuery & 0x01);
	memset(&buffer, 0, sizeof(buffer));
	...
	IT7260_DBG("=IT7260_Init2 --[%x][%x][%x][%x][%x][%x]=\n", buffer[0], buffer[1],
			buffer[2], buffer[3], buffer[4], buffer[5]);
					 ...
					 const struct i2c_device_id *id)
{
	...
	error = request_threaded_irq(client->irq, NULL, ctp_it7260_ts_isr,
	...
	pixcir_early_suspend.suspend = ctp_it7260_ts_suspend;
	pixcir_early_suspend.resume	= ctp_it7260_ts_resume;
...
err_free_irq:
	...
	sprd_free_gpio_irq(ctp_it7260_irq);
err_free_mem:
...
static int __init pixcir_i2c_ts_init(void)
{
	...
	if (ret) {
		IT7260_DBG(KERN_ERR "%s:register chrdev failed\n",__FILE__);
		return ret;
	}
