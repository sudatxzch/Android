...
#else
	return sprintf(buf,
	     __stringify(EV_KEY) ":" __stringify(KEY_HOME)   ":50:500:80:100"
	 ":" __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":265:500:80:100"
	 ":" __stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":160:500:80:100"
	 "\n");
#endif
}
...
static struct kobj_attribute virtual_keys_attr = {
    .attr = {
        .name = "virtualkeys.ft5306_ts", //pixcir_ts
        .mode = S_IRUGO,
    },
...
#if 1//def CONFIG_TOUCHSCREEN_FT5206_WVGA
					 ...
					 const struct i2c_device_id *id)
{
	...
	dev = device_create(i2c_dev_class, &client->adapter->dev, MKDEV(I2C_MAJOR,
			client->adapter->nr), NULL, "ft5306_ts%d", 0);
	if (IS_ERR(dev)) {
		error = PTR_ERR(dev);
		return error;
	}
...
static int pixcir_open(struct inode *inode, struct file *file)
{
	...
	snprintf(client->name, I2C_NAME_SIZE, "ft5306_ts%d", adapter->nr);
...
#if 1
	/*********************************Bee-0928-TOP****************************************/
	ret = register_chrdev(I2C_MAJOR,"ft5306_ts",&pixcir_i2c_ts_fops);
	if (ret) {
#ifdef PIXCIR_DEBUG
...
#if 1
	class_destroy(i2c_dev_class);
	unregister_chrdev(I2C_MAJOR,"ft5306_ts");
#endif
	/********************************Bee-0928-BOTTOM******************************************/
}
